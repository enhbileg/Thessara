...
const UsersPage = () => {
  ...
  const fetchUsers = async () => {
    try {
      const res = await fetch(`/${language}/api/admin/users`);
      const data = await res.json();
      if (data.success) {
        setUsers(data.users);
      } else {
        toast.error(data.message || dict.fetchFailed || "Failed to fetch users");
      }
    } catch (error) {
      toast.error(dict.fetchFailed || "Error fetching users");
    }
  };
  ...
  const clearCart = async (id) => {
    try {
      const res = await fetch(`/${language}/api/admin/users/${id}/clear-cart`, { method: "PUT" });
      const data = await res.json();
      if (data.success) {
        toast.success(dict.clearSuccess || "Cart cleared successfully");
        fetchUsers();
      } else {
        toast.error(data.message || dict.clearFailed || "Failed to clear cart");
      }
    } catch (error) {
      toast.error(dict.clearFailed || "Error clearing cart");
    }
  };

  return (
    <div className="bg-blank border-blank p-6 rounded shadow">
      <h1 className="text-xl font-semibold mb-4 text-specialText">
        {dict.users || "Users"}
      </h1>
        ...
        <table className="w-full border-collapse border border-navbar">
          <thead>
            <tr className="bg-backBanner">
              <th className="border border-navbar px-4 py-2 text-left">
                {dict.image || "Image"}
              </th>
              <th className="border border-navbar px-4 py-2 text-left">
                {dict.name || "Name"}
        ...
        {users.map((user) => (
          ...
          >
            ...
            <p className="text-sm text-gray-700">
              {dict.cartItems || "Cart Items"}: {Object.keys(user.cartItems || {}).length}
            </p>
            <button
              onClick={() => clearCart(user._id)}
